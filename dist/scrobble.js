...
import { entities } from "./generated/model.js";
...
export function parseScrobble(resourceUrl, dataset) {
    ...
    if (data.playedAt instanceof Date && Number.isNaN(data.playedAt.getTime()))
        return undefined;
    const result = data;
    const listener = httpIriOrUndefined(tryRead(() => new Scrobble(scrobbleNodeIris(resourceUrl).event, dataset, DataFactory).listener));
    if (listener !== undefined)
