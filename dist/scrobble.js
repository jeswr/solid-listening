 ...
 */
export function buildScrobble(resourceUrl, data) {
...
export function serializeScrobble(resourceUrl, data) {
 ...
 */
export function parseScrobble(resourceUrl, dataset) {
    ...
    if (instantIri === undefined)
        return undefined;
    const playedAt = tryRead(() => new PlayedAtInstant(instantIri, dataset, DataFactory).dateTime);
    if (playedAt === undefined)
        return undefined;
    const data = { trackTitle, playedAt };
    const artistIri = httpIriOrUndefined(tryRead(() => track.artist));
    if (artistIri !== undefined) {
        setIfDefined(data, "artistName", tryRead(() => new Artist(artistIri, dataset, DataFactory).name));
    }
    const albumIri = httpIriOrUndefined(tryRead(() => track.album));
    if (albumIri !== undefined) {
        setIfDefined(data, "albumTitle", tryRead(() => new Album(albumIri, dataset, DataFactory).title));
    }
    setIfDefined(data, "msPlayed", nonNegativeNumberOrUndefined(tryRead(() => event.msPlayed)));
    setIfDefined(data, "durationSeconds", nonNegativeNumberOrUndefined(tryRead(() => track.durationSeconds)));
    setIfDefined(data, "isrc", tryRead(() => track.isrc));
 ...
 * resolve.
 */
export async function parseScrobbleTtl(resourceUrl, body, contentType = "text/turtle") {
